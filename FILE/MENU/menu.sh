...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/ip/access > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/ip/access | grep $MYIP | awk '{print $2}')
Isadmin=$(curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/ip/access | grep $MYIP | awk '{print $5}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/ip/access | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/ip/access | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
sleep 2
wget -q -O /root/install_up.sh "https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/FILE/PLUGINS/install-up.sh" && chmod +x /root/install_up.sh
sleep 2
./install_up.sh
sleep 5
rm /root/install_up.sh
rm /opt/.ver
version_up=$( curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/FILE/PLUGINS/versi)
echo "$version_up" > /opt/.ver
echo -e "$COLOR1 ${NC}  $COLOR1[INFO]${NC} Successfully Up To Date!"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1 ${NC}              ${WH}• FIRDAUS SIREGAR •${NC}                $COLOR1 $NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
read -n 1 -s -r -p "  Press any key to back on menu"
menu
...
serverV=$( curl -sS https://raw.githubusercontent.com/fsteamID/InstallerPremiumSSH/main/FILE/PLUGINS/versi)
if [ "$Isadmin" = "ON" ]; then
...
echo -e "$COLOR1 $NC ${WH}Memory Usage   ${COLOR1}: ${WH}$uram / 63.9Gi"
# // echo -e "$COLOR1 $NC ${WH}ISP & City     ${COLOR1}: ${WH}$ISP & $CITY"
echo -e "$COLOR1 $NC ${WH}ISP & City     ${COLOR1}: ${WH}OVH Indonesia Raya & $CITY"
...
echo -e "  ${WH}[${COLOR1}40${WH}]${NC} ${COLOR1}• ${WH}Check Support ISP ${WH}[${COLOR1}Menu${WH}] ${WH}[${COLOR1}80${WH}]${NC} ${COLOR1}• ${WH}BOT Panel VPS(WA/Telegram)     ${WH}[${COLOR1}Menu${WH}]  $COLOR1 $NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}"
myver="$(cat /opt/.ver)"
